...
function App() {
  ...
  const fetchTrainings = async (term = '', viewType = 'dashboard') => {
    try {
      ...
      // Fetch all trainings from the backend.
      ...
      // ⭐ Frontend filtering is now based on meeting status, not notification status ⭐
      ...
      if (viewType === 'upcoming') {
        ...
        filteredData = transformedData.filter(t => {
          const dueDate = new Date(t.rawDueDate);
          // A training is 'Ongoing' if it's in the future, and at least one notification has been sent.
          // The transformTrainingData function now returns 'Sent' if any notification was sent.
          // This logic is more accurate. Let's use it for the filter as well.
          const isUpcoming = dueDate > now;
          const anyNotificationsSent = t.notificationStatus === 'Sent'; // Now we can use the transformed status
          return isUpcoming && anyNotificationsSent;
        });
